...
def main():
  ...
  for line in  inputfile:
     ...
     outputfile.write("\tvoid *handle;\n");
     #outputfile.write('\thandle = dlopen("/opt/allinea/20.0.0.0/armpl-20.0.0_ThunderX2CN99_SUSE-12_gcc_9.2.0_aarch64-linux/lib/libarmpl.so", RTLD_LAZY);\n');
     outputfile.write('\thandle = dlopen("/opt/allinea/19.2.0.0/opt/arm/armpl-19.2.0_ThunderX2CN99_SUSE-12_gcc_8.2.0_aarch64-linux/lib/libarmpl.so", RTLD_LAZY);\n');

     # Do the symbol linking (Step 3.c.iv)
     outputfile.write('\t%s = dlsym(handle, "%s");\n' % (prototype.rstrip('\n'), FNNAME))
     ...
     outputfile.write("\tdlclose(handle);\n");
     if (ReturnType != "void") :
